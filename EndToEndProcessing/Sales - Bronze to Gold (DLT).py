...
goldProductSchema = StructType([
    StructField("ProductId", IntegerType()),
    StructField("ProductName", StringType()),
    StructField("PriceEach", DoubleType()),
    StructField("__START_AT", TimestampType()),
    StructField("__END_AT", TimestampType())
])

def salessilver_product_view():
    return(
        spark.readStream
        .format("cloudFiles")
        .options(**bronzeViewConfig)
...
dlt.create_target_table(
    name = "Sales.ProductGold",
    comment = "This is the Product Gold layer table",
    path = "/mnt/adlslirkov/raw/SalesEndToEnd/Gold/Product/",
    schema = goldProductSchema,
    expect_all_or_fail = {"PopulatedId": "ProductId IS NOT NULL", "PopulatedProductName": "ProductName IS NOT NULL", "PopulatedPriceEach": "PriceEach IS NOT NULL"}
)
